...
import com.opencredo.sandbox.gawain.springbatch.remote.chunking.TestItemProcessor;
import com.opencredo.sandbox.gawain.springbatch.remote.util.ExceptionUtil;

public class ChunkMessageChannelItemWriterOrig<T> extends StepExecutionListenerSupport implements ItemWriter<T>,
		ItemStream, StepContributionSource {

	private static final Log logger1 = LogFactory.getLog(TestItemProcessor.class);
	private static final Log logger = LogFactory.getLog(ChunkMessageChannelItemWriterOrig.class);

	static final String ACTUAL = ChunkMessageChannelItemWriterOrig.class.getName() + ".ACTUAL";

	static final String EXPECTED = ChunkMessageChannelItemWriterOrig.class.getName() + ".EXPECTED";
	...
	public void write(List<? extends T> items) throws Exception {
		...
		if (!items.isEmpty()) {
			...
			logger1.info("ChunkMessageChannelItemWriter调用堆栈：" + ExceptionUtil.getStackTrace(new Exception("call stack.")));
