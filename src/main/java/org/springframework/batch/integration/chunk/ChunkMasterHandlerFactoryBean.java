 ...
 * 
 */
public class ChunkMasterHandlerFactoryBean<T> implements FactoryBean<ChunkHandler<T>>, InitializingBean {

	protected final static Logger logger = LoggerFactory.getLogger(ChunkMasterHandlerFactoryBean.class);
	 ...
	 * Usually in practice it will be a {@link ChunkMessageChannelItemWriterOrig}.
	 * 
...
//				+ "] because it already has a remote chunk writer.  Use a local writer in the step.");
		...
		if ((itemWriter instanceof ChunkMessageChannelItemWriterOrig<?>) || (itemWriter instanceof ChunkItemMessageMasterWriter<?>)) {
			throw new IllegalStateException("Cannot adapt step [" + step.getName() + "] because it already has a remote chunk writer.  Use a local writer in the step.");
		}
		...
		logger.debug("Has replace the ChunkProcessor with remote for step {}", step);
		return handler;
	...
	private static Object getField(Object target, String name) {
		...
		if (field == null) {
			logger.debug("Could not find field {} on target {}", name, target);
			return null;
		}

		if (logger.isDebugEnabled()) {
			logger.debug("Getting field {} from target {}", name, target);
		}
